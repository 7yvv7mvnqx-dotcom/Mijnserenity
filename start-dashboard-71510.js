...
const BUILD='8.28.1',TOKEN='828100',ROOT='ms8210Start';
...
function announceReady(){if(readyAnnounced)return;readyAnnounced=true;window.__msApprovedHomeReady8281=true;window.dispatchEvent(new CustomEvent('mijnserenity:dashboard-ready',{detail:{build:BUILD,approved:true,singleRenderer:true,source:'start-dashboard-8281'}}))}
function installStyle(){let s=$('ms8266StartGuardStyle');if(!s){s=document.createElement('style');s.id='ms8266StartGuardStyle';document.head.appendChild(s)}s.textContent=`body.ms8263-home-active>.bottom-nav,body.ms8263-home-active #appView>.bottom-nav,body.ms8263-home-active .bottom-nav.ms8214-nav{display:none!important;visibility:hidden!important;pointer-events:none!important}#dashboard>#ms71510Start,#dashboard>#serenityIvms,#dashboard>#ms71510Dashboard,#dashboard>#msMarineGlass,#dashboard>#msStartCockpit7144,#dashboard>#msDashboardAnalog7141,#dashboard>#msWelcomeCard7140{display:none!important;visibility:hidden!important;pointer-events:none!important}`}
function retireLegacy(){document.body?.classList.remove('ivms-dashboard-active');document.querySelector('.bottom-nav')?.classList.remove('ivms-dashboard-hidden');['serenityIvms','ms71510Start','ms71510Dashboard','msMarineGlass','msStartCockpit7144','msDashboardAnalog7141','msWelcomeCard7140'].forEach(id=>{const el=$(id);if(el){el.dataset.ms8281Retired='1';el.style.setProperty('display','none','important');el.style.setProperty('visibility','hidden','important')}});document.getElementById('dashboard')?.classList.remove('mg-active','scd-active','mspro-active')}
function ensureRoot(){if(route()!=='dashboard')return null;const dashboard=$('dashboard');if(!dashboard)return null;retireLegacy();let root=$(ROOT);if(!root){root=document.createElement('section');root.id=ROOT;dashboard.prepend(root)}dashboard.classList.add('ms8255-reference-dashboard');dashboard.classList.remove('mg-active','scd-active','mspro-active');[...dashboard.children].forEach(el=>{if(el!==root){el.dataset.ms8281Hidden='1';el.style.setProperty('display','none','important')}});root.style.setProperty('display','block','important');root.style.setProperty('visibility','visible','important');if(!root.innerHTML.trim())root.innerHTML='<div style="min-height:100dvh;background:#03131f"></div>';document.body?.classList.add('ms8263-home-active');return root}
...
function ensureNordkapp(){if(window.__msNordkappGen38280)return;load('/nordkapp-gen3-8280.js',NORDKAPP)}
function finish(){retireLegacy();try{window.ms8263ApplyApprovedDashboard?.()}catch(_){};ensurePatch();ensureLive();ensureVictron();ensureSettings();ensureAi();ensureNordkapp();syncBuild();announceReady()}
function apply(){installStyle();retireLegacy();ensureSettings();ensureNordkapp();const root=ensureRoot();if(!root)return false;syncBuild();if(typeof window.ms8263ApplyApprovedDashboard==='function'){try{const ok=!!window.ms8263ApplyApprovedDashboard();ensurePatch();ensureLive();ensureVictron();ensureAi();if(ok)announceReady();return ok}catch(e){console.warn('Approved Haven dashboard failed',e)}}load('/approved-dashboard-8263.js',APPROVED,finish);return false}
function showHome(){try{history.replaceState(null,'',location.pathname+location.search+'#dashboard')}catch(_){};setTimeout(apply,0)}
function boot(){syncBuild();installStyle();retireLegacy();ensureSettings();ensureNordkapp();if(!deepLink())showHome();else if(route()==='dashboard')apply();[80,250,700,1600].forEach(ms=>setTimeout(()=>{retireLegacy();ensureSettings();ensureNordkapp();if(route()==='dashboard')apply()},ms))}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('pageshow',()=>{syncBuild();installStyle();retireLegacy();ensureSettings();ensureNordkapp();if(route()==='dashboard')setTimeout(apply,0)},{passive:true});
window.addEventListener('hashchange',()=>{retireLegacy();ensureSettings();ensureNordkapp();if(route()==='dashboard')setTimeout(apply,0);else document.body?.classList.remove('ms8263-home-active')},{passive:true});
