/* MijnSerenity 8.28.0 — instelbare alarmen + Victron tank/alarm sync + veilige iOS antwoordstem + Nordkapp Gen 3. */
(()=>{
'use strict';
...
function announceReady(){if(readyAnnounced)return;readyAnnounced=true;window.__msApprovedHomeReady8279=true;window.dispatchEvent(new CustomEvent('mijnserenity:dashboard-ready',{detail:{build:BUILD,approved:true,singleRenderer:true,source:'start-dashboard-8280'}}))}
...
function finish(){try{window.ms8263ApplyApprovedDashboard?.()}catch(_){};ensurePatch();ensureLive();ensureVictron();ensureSettings();ensureAi();ensureNordkapp();syncBuild();announceReady()}
function apply(){installStyle();ensureSettings();ensureNordkapp();const root=ensureRoot();if(!root)return false;syncBuild();if(typeof window.ms8263ApplyApprovedDashboard==='function'){try{const ok=!!window.ms8263ApplyApprovedDashboard();ensurePatch();ensureLive();ensureVictron();ensureAi();if(ok)announceReady();return ok}catch(e){console.warn('Approved Haven dashboard failed',e)}}load('/approved-dashboard-8263.js',APPROVED,finish);return false}
function showHome(){try{history.replaceState(null,'',location.pathname+location.search+'#dashboard')}catch(_){};setTimeout(apply,0)}
function boot(){syncBuild();installStyle();ensureSettings();ensureNordkapp();if(!deepLink())showHome();else if(route()==='dashboard')apply();[120,500,1400].forEach(ms=>setTimeout(()=>{ensureSettings();ensureNordkapp();if(route()==='dashboard')apply()},ms))}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('pageshow',()=>{syncBuild();ensureSettings();ensureNordkapp();if(route()==='dashboard')setTimeout(apply,0)},{passive:true});
window.addEventListener('hashchange',()=>{ensureSettings();ensureNordkapp();if(route()==='dashboard')setTimeout(apply,0);else document.body?.classList.remove('ms8263-home-active')},{passive:true});
