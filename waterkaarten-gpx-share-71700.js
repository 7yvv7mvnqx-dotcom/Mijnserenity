/* MijnSerenity 7.17.0 — Waterkaarten via GPX, zonder iframe/API en zonder permanente DOM-scans */
(()=>{
  'use strict';
  if(window.__msWaterkaartenGpx71700)return;
  window.__msWaterkaartenGpx71700=true;

  const BUILD='7.17.0';
  const BUTTON_ID='msnWaterkaarten';
  const STYLE_ID='msWaterkaarten71700Style';
  ...
  async function shareRoute(event){
    ...
    if(points.length<2){
      alert('Bereken eerst een route in MijnSerenity. Daarna kan de route als GPX naar Waterkaarten.');
      return false;
    }
    ...
    try{
      if(window.MijnSerenityNative?.shareGpx){
        ...
        toast('Kies Waterkaarten om de GPX-route te importeren.');
        return false;
      }
  ...
  function installStyle(){
    ...
    style.id=STYLE_ID;
    style.textContent=`#${BUTTON_ID}{display:inline-flex;align-items:center;justify-content:center;gap:7px}#${BUTTON_ID} .mswk-icon{font-size:16px;line-height:1}`;
  ...
  function ensureButton(){
    ...
    if(!tools){
      ...
      if(legend){head.insertBefore(tools,legend);tools.appendChild(legend)}
    ...
    if(!button){
      ...
      button.id=BUTTON_ID;
      button.type='button';
      button.addEventListener('click',shareRoute);
    ...
    if(button.dataset.ms717!=='1'){
      button.innerHTML='<span class="mswk-icon" aria-hidden="true">🗺️</span><b>Route → Waterkaarten</b>';
      button.dataset.ms717='1';
    }
    return true;
  }
  ...
  function scheduleFiniteRetries(){
    [0,250,750,1500,3000,6000].forEach(delay=>setTimeout(ensureButton,delay));
  }
  ...
  function start(){
    ...
    removeLegacyOnce();
    patchGlobals();
    scheduleFiniteRetries();
    window.addEventListener('mijnserenity:routechange',ensureButton,{passive:true});
    window.addEventListener('mijnserenity:native-ready',patchGlobals,{passive:true});
