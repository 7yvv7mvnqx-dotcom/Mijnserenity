...
exports.handler=async(event)=>{
  ...
  try{
    ...
    if(!response.ok){
      ...
      console.warn('Captain AI OpenAI-fout:',response.status,detail.slice(0,500));
      return json(502,{error:'Captain AI kon de analyse niet afronden.'});
    }

    const data=await response.json();
    const answer=outputText(data);
    if(!answer)return json(502,{error:'Captain AI gaf geen antwoord.'});
    return json(200,{ai:true,answer});
  }catch(error){
