...
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(CORE.map(async path=>{try{const response=await fresh(path);await put(cache,path,response)}catch(_){}}));
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('mijnserenity-')&&k!==CACHE_NAME).map(k=>caches.delete(k)));
...
async function networkFirst(request,fallback){
  ...
  try{
    ...
    if(response.ok){await put(cache,request,response);if(fallback)await put(cache,fallback,response)}
    return response;
  }catch(_){
    return (await caches.match(request,{ignoreSearch:false}))||(fallback?await caches.match(fallback):null)||new Response('MijnSerenity kon niet worden geladen.',{status:503,headers:{'content-type':'text/plain; charset=utf-8'}});
  }
}

async function staleWhileRevalidate(request){
  const cached=await caches.match(request,{ignoreSearch:false});
  const update=fresh(request).then(async response=>{if(response.ok){const cache=await caches.open(CACHE_NAME);await put(cache,request,response)}return response}).catch(()=>null);
  return cached||(await update)||new Response('',{status:503});
}

const CRITICAL=new Set([
  '/auth-bootstrap.js',
  '/start-dashboard-71510.js',
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.startsWith('/api/')||url.pathname.startsWith('/.netlify/functions/')||url.pathname.startsWith('/victron-gui/'))return;
  if(request.mode==='navigate'){event.respondWith(networkFirst(request,'/index.html'));return}
  if(CRITICAL.has(url.pathname)){event.respondWith(networkFirst(request));return}
  if(url.pathname.endsWith('.js')||url.pathname.endsWith('.css')||url.pathname==='/manifest.json'){event.respondWith(staleWhileRevalidate(request));return}
...
self.addEventListener('notificationclick',event=>{
  ...
  event.waitUntil((async()=>{
    ...
    for(const client of windows){try{if('navigate' in client)await client.navigate(target);client.postMessage({type:'mijnserenity-open-alarm',data:event.notification?.data||{}});if('focus' in client)return client.focus()}catch(_){}}
