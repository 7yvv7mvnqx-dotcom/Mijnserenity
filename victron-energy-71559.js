...
(()=>{
  ...
  function markup(){return `
    <div id="msMarineGlass" data-ms-victron-live="7195" aria-label="Serenity Victron Live">
      ...
      <div class="mg-main">
        ...
        <div class="mg-mini use"><small>💡 Verbruik</small><strong id="mg7195Load">– W</strong><em>Actueel totaal</em></div>
  ...
  function locateHost(){
    ...
    if(existing&&existing.querySelector('#msMarineGlass'))return existing;
    if(existing&&/^energie\s*&\s*stroom$/im.test(String(existing.innerText||existing.textContent||'')))return existing;
    return findLegacyEnergyCard();
  }
  ...
  function hideDuplicateTankSystems(){
    ...
    if(!root)return;
    const headings=[...root.querySelectorAll('h1,h2,h3,h4,h5,h6,strong,.card-title,.section-title')];
    const heading=headings.find(el=>/^(tanks\s*&\s*systems|tanks?\s*&\s*systemen|tanks?\s+en\s+systemen)$/i.test(String(el.textContent||'').trim()));
    if(!heading)return;
    let card=heading.closest('.card,section,article');
    if(!card||card===root||card.id==='msVictronEnergy'||card.contains($('msMarineGlass'))){
      card=null;
      let node=heading.parentElement;
      for(let depth=0;node&&node!==root&&depth<6;depth++,node=node.parentElement){
        const txt=String(node.innerText||node.textContent||'');
        if(/drinkwater/i.test(txt)&&/dieseltank/i.test(txt)){card=node;break}
      }
    }
  ...
  function render(){
    ...
    set('mg7195Soc',finite(soc)?`${Math.round(soc)}%`:'–%');
    ...
    set('mg-fuel',finite(s.fuel)?`${Math.round(Number(s.fuel))}%`:'–%');set('mg-fuel-l',finite(s.fuelLiters)?`${Math.round(Number(s.fuelLiters))} L`:'– L');
    ...
    const pill=$('mgLivePill');if(pill)pill.classList.toggle('offline',!fresh);const foot=$('mgFoot');if(foot)foot.classList.toggle('offline',!fresh);
  ...
  async function refreshLive(force=false){
    if(liveBusy)return false;if(!force&&Date.now()-lastLiveAttempt<55000)return false;
    const supabase=client(),activeBoat=boat(),activeUser=user(),vrmToken=token();if(!supabase||!activeBoat?.id||!activeUser||!vrmToken)return false;
    ...
    try{
      ...
      window.MIJSERENITY_VRM_LIVE_ENERGY=data;window.dispatchEvent(new CustomEvent('mijnserenity-vrm-energy-live-updated',{detail:data}));queueRender();return true;
    }catch(error){console.warn('Victron live energie kon niet worden vernieuwd:',error);return false}finally{liveBusy=false}
  }

  const events=['mijnserenity-ha-state-updated','mijnserenity-ha-connected','mijnserenity-vrm-updated','mijnserenity-ruuvi-vrm-updated','mijnserenity-vrm-diagnostics-updated','mijnserenity-vrm-energy-live-updated','mijnserenity:dashboard-ready','mijnserenity:routechange'];
