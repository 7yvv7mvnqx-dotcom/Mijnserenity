/* MijnSerenity 7.18.14 — centrale Victron energiestatus + dashboard sync */
(function(){
'use strict';
...
function read(){
  ...
  const chargerPower=tv(charger)??num(t.chargerPower);
  const inverterPower=tv(inverter)??num(t.inverterPower);
  const measuredLoad=tv(load)??num(t.loadPower);
  return {
    ...
    hasVictron:Boolean(soc||voltage||current||power||solar||start||shoreEntity||shoreVoltageEntity||charger||inverter||load||num(directBattery.soc?.value)!==null||num(directBattery.voltage?.value)!==null)
  };
}
...
function syncDashboard(s){
  ...
  const charge=s.power!==null&&s.power>0?s.power:0;
...
function render(){
  ...
  setText('msVictronShore',s.shore===true?'aangesloten':s.shore===false?'niet aangesloten':s.charger!==null?'lader gekoppeld':'niet gekoppeld');
...
['mijnserenity-ha-state-updated','mijnserenity-ha-connected','mijnserenity-vrm-updated','mijnserenity-ruuvi-vrm-updated','mijnserenity-vrm-diagnostics-updated','mijnserenity:routechange'].forEach(name=>window.addEventListener(name,()=>requestAnimationFrame(render),{passive:true}));
window.addEventListener('focus',render);
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{mount();observeDashboard()},{once:true});else{mount();observeDashboard()}
