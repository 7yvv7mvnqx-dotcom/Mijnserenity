/* MijnSerenity 8.28.7 — stabiele dashboardnavigatie op alle vervolgpagina's. */
(()=>{
'use strict';
if(window.__msGlobalDashboardNav8287)return;
window.__msGlobalDashboardNav8287=true;

const BUILD='8.28.7';
const NAV_ID='ms8287GlobalNav';
const STYLE_ID='ms8287GlobalNavStyle';
const MORE_ID='ms8287MoreSheet';
let observer=null;
let lastRequested='';
...
function installStyle(){
  ...
  s.textContent=`
body.ms8287-global-nav-installed .bottom-nav,
body.ms8287-global-nav-installed #appView>.bottom-nav,
body.ms8287-global-nav-installed .bottom-nav.ms8214-nav,
body.ms8287-global-nav-installed #appView>.tabs{display:none!important;visibility:hidden!important;pointer-events:none!important}
body.ms8287-global-nav-active{padding-bottom:calc(104px + env(safe-area-inset-bottom))!important}
...
#${MORE_ID} .ms8287-more-card{width:min(560px,100%);padding:18px;border:1px solid rgba(92,204,244,.30);border-radius:25px;background:#062234;box-shadow:0 24px 80px rgba(0,0,0,.42)}
#${MORE_ID} .ms8287-more-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:13px;color:#fff;font:800 20px/1.2 Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
#${MORE_ID} .ms8287-more-close{width:46px;height:46px;display:grid;place-items:center;border:1px solid rgba(119,210,244,.22);border-radius:50%;background:#082c42;color:#fff;padding:0}
#${MORE_ID} .ms8287-more-close svg{width:22px;height:22px;fill:none;stroke:currentColor;stroke-width:2}
#${MORE_ID} .ms8287-more-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
#${MORE_ID} .ms8287-more-grid button{min-height:54px;padding:10px 12px;border:1px solid rgba(93,203,241,.24);border-radius:15px;background:#0a344b;color:#fff;font-weight:800}
body.ms8284-day #${MORE_ID} .ms8287-more-card{background:#f7fcfd;border-color:rgba(45,146,185,.24)}
body.ms8284-day #${MORE_ID} .ms8287-more-head{color:#08293c}
body.ms8284-day #${MORE_ID} .ms8287-more-close{background:#e4f4f8;color:#0a5168;border-color:rgba(45,146,185,.24)}
body.ms8284-day #${MORE_ID} .ms8287-more-grid button{background:#e9f6fa;color:#0a3449;border-color:rgba(45,146,185,.24)}
...
function navHtml(){return `<nav id="${NAV_ID}" aria-label="Hoofdnavigatie"><button data-ms8287-go="dashboard">${icon.home}<span>Haven</span></button><button data-ms8287-go="map">${icon.map}<span>Kaart</span></button><button data-ms8287-go="planner">${icon.route}<span>Reisplanner</span></button><button data-ms8287-go="ais">${icon.radar}<span>AIS</span></button><button data-ms8287-go="weather">${icon.sun}<span>Weer</span></button><button data-ms8287-go="more">${icon.more}<span>Meer</span></button></nav>`}
function moreHtml(){return `<div id="${MORE_ID}" aria-hidden="true"><div class="ms8287-more-card"><div class="ms8287-more-head"><strong>Meer</strong><button class="ms8287-more-close" aria-label="Sluiten">${icon.close}</button></div><div class="ms8287-more-grid"><button data-ms8287-go="live">Live varen</button><button data-ms8287-go="technical">Techniek</button><button data-ms8287-go="logbook">Logboek</button><button data-ms8287-go="entertainment">Entertainment</button><button data-ms8287-go="pois">POI's</button><button data-ms8287-go="costs">Kosten</button><button data-ms8287-go="finance">Financieel</button><button data-ms8287-go="settings">Instellingen</button></div></div></div>`}

function mount(){
  installStyle();syncBuild();
  document.body?.classList.add('ms8287-global-nav-installed');
...
function bind(){
  ...
  if(nav&&nav.dataset.bound!=='1'){nav.dataset.bound='1';nav.addEventListener('click',e=>{const b=e.target.closest('[data-ms8287-go]');if(b)go(b.dataset.ms8287Go)})}
  if(sheet&&sheet.dataset.bound!=='1'){sheet.dataset.bound='1';sheet.addEventListener('click',e=>{if(e.target===sheet||e.target.closest('.ms8287-more-close')){closeMore();return}const b=e.target.closest('[data-ms8287-go]');if(b)go(b.dataset.ms8287Go)})}
}
...
function go(target){
  if(!target)return;if(target==='more'){openMore();return}
  closeMore();lastRequested=target;lastRequestedAt=Date.now();
  try{history.replaceState(null,'',location.pathname+location.search+'#'+target)}catch(_){}
  syncVisibility();
  if(target==='dashboard'){
    try{if(typeof window.ms8263Navigate==='function'){window.ms8263Navigate('dashboard');setTimeout(()=>{lastRequested='';syncVisibility()},250);return}}catch(_){}
  }
  try{if(typeof window.captainNavigate==='function'){window.captainNavigate(target);setTimeout(()=>{lastRequested='';syncVisibility()},250);return}}catch(_){}
  try{if(typeof window.navigateTo==='function'){window.navigateTo(target);setTimeout(()=>{lastRequested='';syncVisibility()},250);return}}catch(_){}
  try{if(typeof window.showPage==='function'){window.showPage(target);setTimeout(()=>{lastRequested='';syncVisibility()},250);return}}catch(_){}
  setTimeout(()=>{lastRequested='';syncVisibility()},250);
}
...
function syncVisibility(){
  ...
  document.body?.classList.toggle('ms8287-global-nav-active',!onDashboard);
  const active=activeGroup(current);
  nav.querySelectorAll('[data-ms8287-go]').forEach(b=>b.classList.toggle('active',b.dataset.ms8287Go===active));
  if(onDashboard)closeMore();
}

function watch(){
  if(observer||!document.body)return;
  observer=new MutationObserver(()=>{if(!document.getElementById(NAV_ID)||!document.getElementById(MORE_ID)){observer.disconnect();observer=null;mount();return}syncVisibility()});
  observer.observe(document.body,{childList:true,subtree:false,attributes:true,attributeFilter:['class']});
}

window.ms8287ApplyGlobalDashboardNav=()=>{mount();syncVisibility()};
...
setInterval(()=>{if(!document.hidden){mount();syncVisibility()}},1200);
})();
