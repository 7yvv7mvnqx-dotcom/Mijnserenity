/* MijnSerenity 7.18.6 — veilig herstel van Start/dashboard */
(()=>{
  'use strict';
  if(window.__msMarineGlassStartFix71806)return;
  window.__msMarineGlassStartFix71806=true;
  const BUILD='7.18.6';
  ...
  function showDashboard(){
    ...
    if(glass){
      ...
      ['ms71510Dashboard','serenityIvms'].forEach(id=>{
        ...
        if(el){el.hidden=false;el.style.removeProperty('display');el.style.removeProperty('visibility');el.style.removeProperty('opacity')}
      });
    }
  ...
  function start(){
    ...
    if(app&&window.MutationObserver){
      const observer=new MutationObserver(()=>{if(!app.classList.contains('hidden'))requestAnimationFrame(repair)});
