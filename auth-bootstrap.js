/* MijnSerenity 7.19.6 — stabiliteitsbootstrap
   Eén dashboard, één navigatielaag en pagina-uitbreidingen alleen op aanvraag. */
(()=>{
  'use strict';
  if(window.__msBootstrap71960)return;
  window.__msBootstrap71960=true;
  window.__msDisableLegacyVisuals=true;

  const BUILD='7.19.6';
  const VERSION='719060';
  ...
  function removeLegacyLayoutLayers(){
    const blocked=[
      ...
      'marine-glass-polish-7185.css','serenity-control-dashboard.css'
    ];
  ...
  function loadScript(src,timeoutMs=20000){
    ...
    return new Promise((resolve,reject)=>{
      ...
      const finish=error=>{
        ...
        if(error){script.remove();reject(error)}else{loaded.add(pathname);resolve()}
      };
      script.src=src;
      script.async=false;
