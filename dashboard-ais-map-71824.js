/* MijnSerenity 7.18.24 — live AIS in de bovenste dashboardkaart */
(()=>{
  'use strict';
  if(window.__msDashboardAis71824)return;
  window.__msDashboardAis71824=true;
  ...
  function livePosition(){
    ...
    const direct={
      lat:num(state.currentLat??state.lat??state.position?.lat??state.position?.latitude),
      lon:num(state.currentLon??state.lon??state.lng??state.position?.lon??state.position?.lng??state.position?.longitude),
      accuracy:num(state.accuracy??state.gpsAccuracy??state.position?.accuracy)
    };
    if(direct.lat!==null&&direct.lon!==null)return {...direct,timestamp:Date.now(),source:'Live GPS Serenity'};
    ...
    for(const list of sets){
      ...
      const lat=num(point.lat??point.latitude),lon=num(point.lon??point.lng??point.longitude);
      if(lat===null||lon===null)continue;
      return {lat,lon,accuracy:num(point.accuracy??state.accuracy),timestamp:num(point.time??point.timestamp)??Date.now(),source:'Live GPS Serenity'};
    }
    return null;
  }
  ...
  function widgetDocument(position){
    ...
    return `<!doctype html><html lang="nl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"><style>html,body{width:100%;height:100%;margin:0;overflow:hidden;background:#061827}iframe,object,embed{display:block!important;width:100%!important;height:100%!important;border:0!important}</style></head><body><script>var mst_width="100%";var mst_height="100%";var mst_border="0";var mst_map_style="simple";var mst_mmsi="";var mst_show_track="false";var mst_show_info="true";var mst_fleet="";var mst_lat="${lat}";var mst_lng="${lon}";var mst_zoom="13";var mst_show_names="1";var mst_scroll_wheel="true";var mst_show_menu="false";(function(){var send=function(state,message){parent.postMessage({source:'mijnserenity-dashboard-ais',state:state,message:message||''},'*')};window.addEventListener('error',function(event){send('error',event.message||'AIS-widget fout')});var observer=new MutationObserver(function(){var map=document.querySelector('iframe,object,embed');if(!map)return;map.style.width='100%';map.style.height='100%';map.style.border='0';observer.disconnect();send('ready','')});observer.observe(document.documentElement,{childList:true,subtree:true});setTimeout(function(){send('waiting','')},10000)})();<\/script><script src="${WIDGET_SCRIPT}" async defer onload="parent.postMessage({source:'mijnserenity-dashboard-ais',state:'script-loaded',message:''},'*')" onerror="parent.postMessage({source:'mijnserenity-dashboard-ais',state:'error',message:'AIS-bron kon niet worden geladen'},'*')"><\/script></body></html>`;
  }

  function ensureStyle(){
    if($('mgDashboardAis71824Style'))return;
    const style=document.createElement('style');
    style.id='mgDashboardAis71824Style';
    style.textContent=`
      ...
      #msMarineGlass .mg-ais-live-layer{position:absolute;inset:0;z-index:700;background:#061827;display:none}
      ...
      @media(max-width:760px){#msMarineGlass .mg-map-mode-switch{top:9px}#msMarineGlass .mg-map-mode-switch button{min-height:32px!important;padding:0 11px!important;font-size:10px!important}#msMarineGlass .mg-ais-overlay{bottom:8px;left:8px;right:8px}#msMarineGlass .mg-ais-status{max-width:67%;font-size:9px;padding:6px 8px}.mg-ais-open{font-size:9px!important}}
    `;
  ...
  function ensureUi(){
    ...
    if(!$('mgMapModeSwitch')){
      ...
      switcher.addEventListener('click',event=>{
        ...
        if(!button)return;
        event.preventDefault();event.stopPropagation();setMode(button.dataset.mode,true);
      });
    ...
    if(!$('mgAisLiveLayer')){
      ...
      $('mgAisOpenFull')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(typeof window.captainNavigate==='function')window.captainNavigate('ais');else window.ms708GoToPage?.('ais',true)});
    }
    return card;
  }
  ...
  function bindMessages(){
    ...
    window.addEventListener('message',event=>{
      ...
      if(data.state==='ready'){
        ...
        status('AIS laden duurt langer dan normaal');
      }
    });
  }
  ...
  function refreshIfNeeded(){
    ...
    if(next){currentPosition=next;savePosition(next)}
    loadAis(false);
  }
