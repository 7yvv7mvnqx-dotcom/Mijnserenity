/* MijnSerenity 7.17.0 — geoptimaliseerde professionele cockpit */
(()=>{
'use strict';
if(window.__msProCockpit71700)return;window.__msProCockpit71700=true;
const BUILD='7.17.0';
...
let navObserver=null,syncTimer=null,navRoot=null;

function showDashboard(){
  const host=$('dashboard');if(!host)return;
  host.classList.add('mspro-active','active');host.hidden=false;host.style.removeProperty('display');
  const pro=$('msProDashboard');if(pro){pro.hidden=false;pro.style.display='block'}
}

function go(route){
  if(route==='dashboard'){
    showDashboard();
    document.querySelectorAll('.page').forEach(p=>{if(p!==$('dashboard'))p.classList.remove('active')});
    return;
  }
...
function ensureNav(){
  ...
  if(nav.dataset.msCockpitBound!=='1'){
    nav.dataset.msCockpitBound='1';
    nav.addEventListener('click',e=>{
      const b=e.target.closest('.bottom-nav-item');if(!b)return;
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
      nav.querySelectorAll('.bottom-nav-item').forEach(x=>x.classList.toggle('active',x===b));
      go(b.dataset.target);
    },true);
  }
}
...
function installNavGuard(){
  ...
  navRoot=nav?.parentElement||document.body;
  navObserver=new MutationObserver(()=>requestAnimationFrame(ensureNav));
  /* Alleen directe navigatiewijzigingen observeren; niet langer de volledige DOM-subtree. */
  navObserver.observe(navRoot,{childList:true,subtree:false});
}

function build(){
  const host=$('dashboard');if(!host)return;
  host.classList.add('mspro-active');
  if($('msProDashboard')){showDashboard();return}
  ...
  el.addEventListener('click',e=>{const b=e.target.closest('[data-go]');if(b)go(b.dataset.go)});
  showDashboard();
}

function coords(){
  const pts=window.liveNavState?.points,p=Array.isArray(pts)&&pts.length?pts[pts.length-1]:null;
...
function windFrom(){
  ...
  const s=txt('ivmsWeatherWind')||txt('ms71512WindDirection'),m=s.match(/(\d{1,3})\s*°/);
...
function sync(){
  ...
  const from=windFrom(),heading=norm(window.__msWindDeviceHeading),to=from===null?null:norm(from+180),rotation=to===null?0:(heading===null?to:norm(to-heading));
  if(from!==null){
    set('mscWindDeg',String(Math.round(from)).padStart(3,'0')+'°');
    set('mscAwa',String(Math.round(from)).padStart(3,'0')+'°');
    const a=$('mscWindArrow');if(a)a.style.transform='translate(-50%,-100%) rotate('+rotation+'deg)';
  }
  const ws=num(txt('ivmsWindValue'))??num(window.liveNavState?.weather?.windSpeed);
  if(ws!==null){set('mscAws',(ws/1.852).toFixed(1)+' kn');set('mscTws',ws.toFixed(1));set('mscNowWind',(ws/1.852).toFixed(0)+' kn')}
  ...
  let rud=num($('liveRudderInput')?.value)??num(window.liveNavState?.rudderAngle)??0;rud=Math.max(-35,Math.min(35,rud));
  set('mscRudder',Math.round(rud)+'°');
  const rn=$('mscRudderNeedle');if(rn)rn.style.transform='translateX(-50%) rotate('+rud+'deg)';
  const depth=num(txt('ivmsDepth'))??num(txt('ms71510Depth'));
  set('mscDepth',depth===null?'–':depth.toFixed(1));set('mscDepthState',depth===null?'Niet gekoppeld':'Actief');
  set('mscWater',read(['ms71510WaterTemp','ms793WeatherWaterTemp','rwsWaterTemperature'],'– °C'));
  set('mscSoc',read(['ivmsHouseSoc','techHouseSoc','haHouseSoc'],'–%'));
  const volt=read(['ivmsHouseVoltage','techHouseVoltage'],'– V'),amp=read(['ivmsHouseCurrent','haHouseCurrent'],'– A');
  set('mscVoltAmp',volt+'　'+amp);set('mscAmp',amp);
  set('mscShore',read(['ivmsShorePower','techShorePower'],'–'));
  set('mscSolar',read(['ivmsSolarPower','techSolarPower'],'– W'));
...
function init(){
  build();installNavGuard();showDashboard();sync();
  setTimeout(()=>{if(dashboardVisible())sync()},350);
  ['mijnserenity-ha-state-updated','mijnserenity-ha-connected','mijnserenity-ruuvi-vrm-updated','mijnserenity:routechange'].forEach(n=>window.addEventListener(n,eventSync,{passive:true}));
